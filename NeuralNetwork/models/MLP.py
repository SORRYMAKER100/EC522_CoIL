...
class MLP(object):
    ...
    def train(self, output_path, train_provider, valid_provider, batch_size=20, valid_size=20,
              epochs=80, learning_rate=0.001, is_restore=False, prediction_path='predict',
              save_epoch=10, patience=30, min_delta=0.005):

        abs_output_path, abs_prediction_path = self._path_checker(output_path, prediction_path, is_restore)
        ...
        with tf.Session(config=self.config) as sess:
            ...
            summary_writer = tf.summary.FileWriter(abs_output_path, graph=sess.graph)
            ...
            for epoch in range(epochs):
                ...
                for iter in range(iters_per_epoch):
                    ...
                    lr = learning_rate[epoch] if type(learning_rate) is np.ndarray else learning_rate
                    _, loss, avg_snr = sess.run([self.optimizer, self.loss, self.avg_snr], feed_dict={
                        self.x: batch_x, self.y: batch_y, self.lr: lr})
                    tf.logging.info("[Epoch {}/{}: {}/{}] Minibatch MSE = {:.4f}, Minibatch SNR = {:.4f}".format(
                        epoch + 1, epochs, iter + 1, iters_per_epoch, loss, avg_snr))

                cur_val_avg_snr = self._output_valstats(sess, summary_writer, epoch, valid_x, valid_y,
                                                        "epoch_{}.mat".format(epoch + 1), abs_prediction_path)

                if cur_val_avg_snr - best > min_delta:
                    best_delta = cur_val_avg_snr - best
                    tf.logging.info("best_delta = {:.4f}".format(best_delta))
                    tf.logging.info("best_epoch = {}".format(best_epoch))
                    best = cur_val_avg_snr
                    best_epoch = epoch
                    self.save(sess, save_path)
                elif epoch - best_epoch >= patience and epoch >= 1200:
                    tf.logging.info("Early stopping at epoch {}".format(epoch + 1))
                    break
                else:
                    tf.logging.info("best_epoch = {}".format(best_epoch))
                if (epoch + 1) % save_epoch == 0:
                    path = os.path.join(abs_output_path, "{}_model/model".format(epoch + 1))
